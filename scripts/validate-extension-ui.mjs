#!/usr/bin/env node
/*
 * Vendored from AiderDesk v0.80.0 (github.com/hotovo/aider-desk,
 * resources/skills/extension-creator/assets/scripts, Apache-2.0) so CI can
 * validate the JSX UI components without a local AiderDesk installation.
 * Re-vendor when bumping the minimum supported AiderDesk version.
 * Local patches: fall back to an aliased typescript@5 when the repo uses a
 * TypeScript >= 7 native build without the legacy compiler API.
 */
/**
...
const resolveEnvironment = () => {
  ...
  // Prefer the repo's own TypeScript; fall back to an aliased typescript@5
  // (local addition) because TypeScript >= 7 ships as a native build whose
  // JS package no longer exposes the legacy compiler API used below.
  const loaded = loadOptional(() => require('typescript'));
  const usable = !!loaded?.ScriptTarget && !!loaded?.createProgram ? loaded : undefined;
  const candidate = usable ?? loadOptional(() => require('typescript5'));
